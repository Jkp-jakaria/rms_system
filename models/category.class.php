...
class Category implements JsonSerializable{
	...
	public function save(){
		global $db,$tx;
		$db->query("insert into {$tx}product_categories(name,created_at,updated_at)values('$this->name','$this->created_at','$this->updated_at')");
		return $db->insert_id;
	}
	public function update(){
		global $db,$tx;
		$db->query("update {$tx}product_categories set name='$this->name',created_at='$this->created_at',updated_at='$this->updated_at' where id='$this->id'");
	}
	...
	public static function all(){
		global $db,$tx;
		$result=$db->query("select id,name,created_at,updated_at from {$tx}product_categories");
		$data=[];
	...
	public static function find($id){
		global $db,$tx;
		$result =$db->query("select id,name,created_at,updated_at from {$tx}product_categories where id='$id'");
...
";
	...
	static function html_table($page = 1,$perpage = 10,$action=true){
		...
		$result=$db->query("select id,name,created_at,updated_at from {$tx}product_categories limit $top,$perpage");
