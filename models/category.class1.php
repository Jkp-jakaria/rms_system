...
";
	...
	static function html_table($page = 1,$perpage = 10,$action=true){
		global $db,$tx;
		$count_result =$db->query("select count(*) total from {$tx}product_categories");
		list($total_rows)=$count_result->fetch_row();
		$total_pages = ceil($total_rows /$perpage);
		$top = ($page - 1)*$perpage;

		// $result=$db->query("select id,name,section_id,created_at,updated_at from {$tx}product_categories limit $top,$perpage");

		$result=$db->query("SELECT pc.id,pc.name,s.name section,pc.created_at,pc.updated_at from {$tx}product_categories pc,{$tx}sections s where s.id=pc.section_id limit $top,$perpage");

		$html="<div class='mb-3'><a class=\"btn btn-success\" href=\"create-category\">New Category</a></div>";
		$html.="<table class='table'>";
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Section</th><th>Created At</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Name</th><th>Section</th><th>Created At</th></tr>";
		}
		while($category=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$category->id</td><td>$category->name</td><td>$category->section</td><td>$category->created_at</td> $action_buttons</tr>";
		}
		$html.="</table>";
