...
class Person implements JsonSerializable{
	...
	public static function pagination($page=1,$perpage=10,$criteria=""){
		global $db,$tx;
		$top=($page-1)*$perpage;
		$result=$db->query("select id,name,position_id,sex,dob,doj,mobile,address,inactive from {$tx}persons $criteria limit $top,$perpage");
		$data=[];
		while($person=$result->fetch_object()){
			$data[]=$person;
		}
...
";
	...
	static function html_table($page = 1,$perpage = 10,$criteria="",$action=true){
		...
		$result=$db->query("SELECT pe.id,pe.name,po.name positions,pe.sex,pe.dob,pe.doj,pe.mobile,pe.address,pe.inactive FROM {$tx}persons pe,{$tx}positions po where po.id=pe.position_id limit $top,$perpage");

		$html="<div class='mb-3'><a class=\"btn btn-success\" href=\"create-person\">New Person</a></div>";
		$html.="<table class='table'>";
		if($action){
			$html.="<tr><th>Id</th><th>Name</th><th>Positions</th><th>Sex</th><th>Dob</th><th>Mobile</th><th>Action</th></tr>";
		}else{
			$html.="<tr><th>Id</th><th>Name</th><th>Positions</th><th>Sex</th><th>Dob</th><th>Mobile</th></tr>";
		}
		while($person=$result->fetch_object()){
			$action_buttons = "";
			if($action){
			...
			$html.="<tr><td>$person->id</td><td>$person->name</td><td>$person->positions</td><td>$person->sex</td><td>$person->dob</td><td>$person->mobile</td>$action_buttons</tr>";
		}
		$html.="</table>";
